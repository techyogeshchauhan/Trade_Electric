...
if ($result && $result->num_rows > 0) {
    while($row = $result->fetch_assoc()) {
        ...
        if ($current_user_role === 'seller' && $row['user_id'] != $current_user_id) {
            $action_btn = "<button class='btn btn-primary btn-sm' onclick='matchNow({$row['id']})'>
                            <i class='bi bi-lightning-fill'></i> Match
                          </button>";
        } else if ($current_user_role === 'buyer' && $row['user_id'] == $current_user_id) {
            $action_btn = "<span class='badge bg-warning text-dark'>Your Demand</span>";
        } else {
